 ...
 * Extracted from pi-main for use in fs-tool.
 */
...
export function generateDiffString(
	...
	for (let i = 0; i < parts.length; i++) {
		...
		// diff-lines always ends with an empty string from the trailing newline
		...
		if (part.added) {
			...
			// Context: show first and last few lines
			const maxContext = contextLines * 2;
			if (lines.length <= maxContext + 2) {
				for (const line of lines) {
					output.push(`${String(newLineNum).padStart(lineNumWidth)}   ${line}`);
					oldLineNum++;
					newLineNum++;
				...
				for (let j = 0; j < contextLines; j++) {
					output.push(
						`${String(newLineNum).padStart(lineNumWidth)}   ${lines[j]}`,
					);
					oldLineNum++;
					newLineNum++;
				}
				const skipped = lines.length - maxContext;
				output.push(
					`${" ".repeat(lineNumWidth)}   ... (${skipped} unchanged lines) ...`,
				);
				oldLineNum += skipped;
				newLineNum += skipped;
